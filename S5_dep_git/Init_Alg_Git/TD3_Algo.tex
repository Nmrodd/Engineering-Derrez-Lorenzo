...
\text{Pour le $1^{er}$ cas :}
\begin{itemize}
    \item Complexité linéaire $\implies$ proportionnel à la taille en octets de l'entrée.
    \item Après calcul on obtient en temps linéaire $96 \times 10^{-9}$ s.
\end{itemize}

\text{Pour le $2^{e}$ cas :} 
\begin{itemize}
    \item Après calcul on obtient en temps exponentiel environ 5 siècles. 
\end{itemize}
...
def longueurMaximalePlateau(T: Array[integer]) -> integer :
    ...
    while(i<n) :
        int j = 1
        while(j<n and est_plateau(i, j, T)) :
            j = j + 1
        max = j - i 
        i = i + 1 
    return max
...
\begin{itemize}
    ...
    \subitem 3 imbrications en lien avec n la taille du tableau $\implies$ $O(n^3)$
